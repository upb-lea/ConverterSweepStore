...
class MainWindow(QMainWindow):
    ...
    def updateScatterRadios(self, button) :         
        ...
        if selected :
            ...
            self.plotScatter(tempFilteredDF,xAxisPoint,selected,xLabel,yLabel,length)
    ...
    def plotScatter(self,df,xkey,ykey,xLabel,yLabel,length):
        try :
            if df.empty:
                self.MplWidget.canvas.figure.set_visible(False)
                self.MplWidget.toolbar.hide()
            ...
            self.xList = df[xkey].tolist()
            temp = defaultdict(lambda: len(temp))
            xMapList = [temp[ele] for ele in self.xList]
            for key, grp in df.groupby(['Datasheet']):
               self.sc['scatter']= grp.plot(ax=self.MplWidget.canvas.axes, kind='scatter', x=xkey, y=ykey, marker='o',grid =True, label=key)
